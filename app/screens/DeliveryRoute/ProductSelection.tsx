...
export const ProductSelectionScreen: FC<ProductSelectionProps> = observer(
  function ProductSelectionScreen(
    ...
    const {
      stockStore,
      orderStore: { updateOrderCart, cart, getTotalItem, getTotalPrice, selectedTask },
    } = useStores()
    ...
    function handleSelectProduct(product) {
      const productCart = cart.find((item) => item.name === product.name)
    ...
    function handleAddToBasket() {
      ...
      updateOrderCart(selectedProduct)
    ...
    return (
      <>
        <View style={{ flex: 1 }}>
          <Header title={"Select Products"} leftIcon={"back"} onLeftPress={handleBack} />
        ...
        <Button
          ...
          LeftAccessory={(props) => (
            <Text
              ...
              weight={"bold"}
            >{`Basket ${getTotalItem()} items`}</Text>
          )}
